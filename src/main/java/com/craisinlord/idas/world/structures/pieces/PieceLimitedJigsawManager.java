...
public class PieceLimitedJigsawManager {
    ...
    public static Optional<Structure.GenerationStub> assembleJigsawStructure(
        ...
        if(startPool.size() == 0) {
            IDAS.LOGGER.warn("IDAS: Empty or nonexistent start pool in structure: {}  Crash is imminent", structureID);
            throw new RuntimeException("IDAS: Empty or nonexistent start pool in structure: " + structureID + " Crash is imminent");
        }
                ...
                if (attempts == 100) {
                    IDAS.LOGGER.error(
                            """
                                    ...
                                    -------------------------------------------------------------------
                                    IDAS: Failed to create valid structure with all required pieces starting from this pool file: {}. Required pieces failed to generate the required amount are: {}
                                      This can happen if a structure has a required piece but the structure size is set too low.
                                      However, this is most likely caused by a structure unable to spawn properly due to hitting the world's min y or max y build thresholds or a broken IDAS datapack.
                                      ...
                                      Please report the issue to IDAS's dev with latest.log file if the structure is not cut off by world min/max y build thresholds.
        ...
        public void generatePiece(PoolElementStructurePiece piece,
            ...
            for (StructureTemplate.StructureBlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().size() != 0 || Objects.equals(jigsawBlockPool, Pools.EMPTY.location())))) {
                    IDAS.LOGGER.warn("IDAS: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockPool, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).idas_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().size() != 0 || Objects.equals(jigsawBlockFallback, Pools.EMPTY.location())))) {
                    IDAS.LOGGER.warn("IDAS: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockFallback, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).idas_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
